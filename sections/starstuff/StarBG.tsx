...
const onLoad = (numberOfStars: number) => {
  ...
  function addStarComponent() { // to do: stars appear at different intervals independently of each other
  ...
  function randomizeLocation(i: number) {
    ...
    const var2 =
      Math.round(Math.random() * (globalThis.innerHeight * 0.6 - 5)) +
      "px";

    const sparkleEl = Array.from(
      document.getElementsByClassName("sparkle1") as HTMLCollectionOf<
        HTMLElement
      >,
    );
